...
# CHEM_REQUIRE_GNU_MAKE
# --------------------
#
# Call AX_CHECK_GNU_MAKE (_not_ CHEM_PROG_GNU_MAKE), and then tell the
...
#
AC_DEFUN([CHEM_REQUIRE_GNU_MAKE],[{ :;
  AX_CHECK_GNU_MAKE
...
# ----------------------------
#
# Call CHEM_PROG_GNU_MAKE, and then tell the user to install GNU Make
# in $PATH, if GNU Make is not found.
#
# If you use this macro, you likely also want to use
# CHEM_CONFIG_MAKEFILES.
#
AC_DEFUN([CHEM_REQUIRE_GNU_MAKE_IN_PATH],[{ :;
  CHEM_PROG_GNU_MAKE
  if test -z "${_cv_gnu_make_command}"; then
